...
var consoleServer = "http://localhost:8080";
  ...
  if (!data.command) {
    ...
    for (var i = 0; i < argt; i++) {
      ...
      if (typeof data.args[i] === "object") {
        ...
        data_out.push(
          pclose +
            jsonStringSyntaxHighlight(data.args[i], pfx_close, obid, ohide)
        );
      } else if (data.args[i]) {
        ...
        if (submatch) {
          for (var s = 1; s < submatch.length + 1; s++) {
            ...
            if (typeof nexobj === "object") {
              ...
              next =
                next +
                jsonStringSyntaxHighlight(nexobj, pfx_close, obid, ohide);
              try {
                if (nexobjEl.element) {
                  pfx_close = "&lt<b>-</b>&gt";
                  next =
                  ...
                  next =
                    next +
                    htmlStringSyntaxHighlight(nexobj, pfx_close, obid, true);
                  has_html = true;
                }
              } catch (e) {}
            } else {
...
$(function () {
  ...
  $("#style-switcher").on("click", function (e) {
    ...
    if (t.is(".cup")) {
      ...
      if (confirm("Do you want to restore default settings?"))
        clearUserConfig();
    }
  });
